...
use Modules\Category\Http\Middleware\AuthorisedApiTokenAdmin;
...
class CategoryServiceProvider extends ServiceProvider
{
    ...
    private function registerBindings()
    {
        ...
        $this->app->bind(
            'Modules\Category\Repositories\UserRepository',
            function () {
                $repository = new \Modules\Category\Repositories\Eloquent\EloquentUserRepository(new \Modules\Category\Entities\User());

                if (! config('app.cache')) {
                    return $repository;
                }

                return new \Modules\Category\Repositories\Cache\CacheUserDecorator($repository);
            }
        );
// add bindings
